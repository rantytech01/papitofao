...
export async function updateSiteSettings(formData: FormData) {
  ...
  const { error } = await supabase.from("site_settings").upsert({
    ...
    primary_color: String(formData.get("primary_color") || "#0757D5"),
    secondary_color: String(formData.get("secondary_color") || "#ED1111"),
    accent_color: String(formData.get("accent_color") || "#071B3A"),
